...
(function () {
  ...
  // ----- Hero typing effect (word by word) -----
  var typed = document.getElementById('typed');
  if (typed && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
    var words = typed.textContent.trim().split(/\s+/), n = 0;
    typed.textContent = '';
    typed.classList.add('typing');
    (function next() {
      typed.textContent = words.slice(0, ++n).join(' ');
      if (n < words.length) setTimeout(next, 600);
      else setTimeout(function () { typed.classList.remove('typing'); }, 2500);
    })();
  }
